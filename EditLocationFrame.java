...
public class EditLocationFrame extends JFrame {
    ...
    public EditLocationFrame(LocationsListFrame parent, int row, Object[] locationData) {
        ...
        idField.setText((String) locationData[0]);
        formPanel.add(idField);

        // Client ID field
        formPanel.add(new JLabel("Client ID:"));
        clientIdField = new JTextField((String) locationData[1]);
        formPanel.add(clientIdField);

        // Car ID field
        formPanel.add(new JLabel("Car ID:"));
        carIdField = new JTextField((String) locationData[2]);
        formPanel.add(carIdField);

        // Start Date field
        formPanel.add(new JLabel("Start Date:"));
        startDateField = new JTextField((String) locationData[4]);
        formPanel.add(startDateField);

        // End Date field
        formPanel.add(new JLabel("End Date:"));
        endDateField = new JTextField((String) locationData[5]);
        formPanel.add(endDateField);

        // Price field
        formPanel.add(new JLabel("Price:"));
        priceField = new JTextField((String) locationData[6]);
        formPanel.add(priceField);

        // Status field
        formPanel.add(new JLabel("Status:"));
        statusComboBox = new JComboBox<>(new String[]{"Active", "Completed", "Cancelled"});
        statusComboBox.setSelectedItem(locationData[7]);
    ...
    private void handleSave() {
        ...
        Object[] updatedLocation = {
            ...
            carIdField.getText(),
            "", // Client Name will be updated by the parent frame
        ...
        parentFrame.updateLocation(editingRow, updatedLocation);
        dispose();
    }
} 